...
Completely standalone data collection script that bypasses all Gaia/SIMBAD issues.
"""
import pandas as pd
import numpy as np
...
def fetch_nasa_exoplanets_standalone():
    """Fetch NASA Exoplanet Archive data with standalone approach"""
    print("=== Fetching NASA Exoplanet Archive (Standalone) ===")
    ...
    try:
        ...
        # Add placeholder coordinates for 3D visualization
        df['x_ly'] = df['distance_ly'] * np.cos(np.radians(df['dec'])) * np.cos(np.radians(df['ra']))
        df['y_ly'] = df['distance_ly'] * np.cos(np.radians(df['dec'])) * np.sin(np.radians(df['ra']))
        df['z_ly'] = df['distance_ly'] * np.sin(np.radians(df['dec']))
        
        # Save the data
        output_file = RAW_DATA_DIR / "nasa_exoplanets_standalone.csv"
...
def fetch_jpl_solar_system_standalone():
    """Fetch Solar System data with standalone approach"""
    print("=== Fetching Solar System Data (Standalone) ===")
    ...
    try:
        ...
        if data_list:
            ...
            output_file = RAW_DATA_DIR / "solar_system_standalone.csv"
            df.to_csv(output_file, index=False)
        ...
        print(f"❌ Error in Solar System data collection: {e}")
        return None

def fetch_messier_catalog_standalone():
    """Fetch Messier Catalog data with standalone approach"""
    print("=== Fetching Messier Catalog (Standalone) ===")
    ...
    if data_list:
        df = pd.DataFrame(data_list)
        output_file = RAW_DATA_DIR / "messier_catalog_standalone.csv"
        df.to_csv(output_file, index=False)
...
def main():
    ...
    print("=== Standalone Data Collection ===")
    print("Bypassing all Gaia/SIMBAD issues and focusing on core data")
    
    # Collect all data
    exoplanets_df = fetch_nasa_exoplanets_standalone()
    solar_system_df = fetch_jpl_solar_system_standalone()
    messier_df = fetch_messier_catalog_standalone()
    ...
    print("\n✓ Standalone data collection completed!")
